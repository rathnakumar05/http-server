...
send_html (int s)
{
  ...
  if (f == NULL)
    {
      close (s);
      error_exit ("Server: Failed to open file");
    }
  ...
  if (fseek (f, 0, SEEK_END) != 0)
    {
      close (s);
      fclose (f);
      error_exit ("Server: Failed to seek file");
    }
  ...
  if (file_size == -1)
    {
      close (s);
      fclose (f);
      error_exit ("Server: Failed to ftell file");
    }
  ...
  if (ferror (f))
    {
      close (s);
      fclose (f);
      error_exit ("Server: Failed to rewind file");
    }
  ...
  send_all (s, buff, strlen (buff));

  while ((file_bytes_read = fread (chunk, 1, sizeof chunk, f)) > 0)
    {
      send_all (s, chunk, file_bytes_read);
    }

  if (ferror (f))
    {
      close (s);
      error_exit ("Server: Failed to read file");
    }
...
void
parse_header (char buff[BUFFER_SIZE])
{
