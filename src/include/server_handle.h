...
int handle_recv(client_connection_t *client_connection);
const char * http_reason_phrase (int code);
const char * get_mime_type (const char *path);
int http_response (client_connection_t * client_connection, char *path);
int send_all (int s, char *buff, size_t buff_len);
int handle_send (client_connection_t * client_connection);
